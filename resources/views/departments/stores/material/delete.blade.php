    ...
    <div class="content-wrapper">
        <div class="row justify-content-center">
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ __('general.breadcrumb.material.material_delete') }}</li>
                    </ol>
                </nav>
            </div>
            <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title"><i class="mdi mdi-trash-can"></i> {{ __('general.breadcrumb.material.material_delete') }}</h4>
                        <h4 class="card-description text-danger">{{ __('material.delete.warning') }}</h4>
                        <form class="needs-validation" action="{{ route('material.destroy',encrypt($id)) }}" method="POST">
                            @csrf
                            {{ method_field('DELETE') }}
                            <button type="submit" class="btn btn-success mr-2">{{ __('general.form.delete') }}</button>
                            <a class="btn btn-light" href="{{ route('material.index') }}">{{ __('general.form.back') }}</a>
                        </form>
                    </div>
                </div>
            </div>
