    ...
    <div class="content-wrapper">
        <div class="row justify-content-center">
            ...
            <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">{{ __('general.breadcrumb.labeltype.labeltype_create') }}</h4>
                        <form class="needs-validation" action="{{ route('labeltype.store') }}" method="POST" novalidate>
                            @csrf
                            <div class="form-group{{ $errors->has('code') ? 'has-error' : '' }}">
                                <label for="name">{{ __('labeltype.code') }} *</label>
                                <input type="text" class="form-control" id="code" name="code" placeholder="Label type code" value="@if(old('code')) {{ old('code') }} @endif" required>
                                @error('code')
                                <p class="text-small text-danger">{{ $errors->first() }}</p>
                                @enderror
                            </div>
                            <div class="form-group{{ $errors->has('name') ? 'has-error' : '' }}">
                                <label for="description">{{ __('labeltype.name') }} *</label>
                                <input type="text" class="form-control" id="name" name="name" placeholder="Label type name" value="@if(old('name')) {{ old('name') }} @endif" required>
                                @error('name')
                                <p class="text-small text-danger">{{ $errors->first() }}</p>
                                @enderror
                            </div>
                            ...
                            <a class="btn btn-light" href="{{ route('labeltype.index') }}">
