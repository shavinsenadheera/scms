    ...
    <div class="content-wrapper">
        <div class="row justify-content-center">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">
                        {!! trans('general.breadcrumb.role.role_delete') !!}
                    </li>
                </ol>
            </nav>
            </div>
            <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title"><i class="mdi mdi-trash-can"></i> {!! trans('general.breadcrumb.role.role_delete') !!}</h4>
                        <h4 class="card-description text-danger">{!! trans('general.form.delete_topic') !!}</h4>
                        <form class="needs-validation" action="{{ route('role.destroy',$id) }}" method="POST">
                            @csrf
                            {{ method_field('DELETE') }}
                            <button type="submit" class="btn btn-success mr-2">{!! trans('general.form.delete') !!}</button>
                            <a class="btn btn-light" href="{{ route('role.index') }}">{!! trans('general.form.delete') !!}</a>
                        </form>
                    </div>
                </div>
            </div>
