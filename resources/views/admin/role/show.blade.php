        ...
        <div class="row justify-content-center">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('general.breadcrumb.role.role_edit') }}</li>
                </ol>
            </nav>
            </div>
            <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title"><i class="mdi mdi-pencil"></i> {{ __('general.breadcrumb.role.role_edit') }}</h4>
                        <form class="needs-validation" action="{{ route('role.update',$role->id) }}" method="POST" novalidate>
                            ...
                            <div class="form-group{{ $errors->has('description') ? 'has-error' : ''}}">
                                ...
                                <input type="text" class="form-control" name="description" id="description" placeholder="Enter description" value="{{ $role->description }}" required>
                                @error('description')
                                <p class="text-small text-danger">{{ $errors->first() }}</p>
                                @enderror
                            </div>
                            <div class="form-group{{ $errors->has('permission') ? 'has-error' : '' }}">
                                <label for="permission">{{ __('role.permissions') }} *</label>
                                <select class="form-control js-example-basic-multiple" name="permission[]" id="permission" multiple data-size="5" required>
                                    @foreach($permissions as $data)
                                        <option {{ in_array($data->id, $gotPermissions) ? 'selected' : '' }} value="{{ $data->id }}">{{ $data->name }}</option>
                                    @endforeach
                                </select>
                                @error('permission')
                                <p class="text-small text-danger">{{ $errors->first() }}</p>
